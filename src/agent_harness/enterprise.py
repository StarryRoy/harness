...
class GuardrailMiddleware(AgentMiddleware):
    ...
    def before_agent(self, execution: AgentExecution) -> None:
        if self.input_guardrail:
            execution.metadata["guardrail"] = "input"
    ...
    def wrap_tool_call(self, request: ToolRequest, call_next: Any) -> Any:
        if self.tool_guardrail:
            request.execution.metadata["guardrail"] = "tool"
    ...
    def after_agent(self, execution: AgentExecution, result: Any) -> Any:
        if self.output_guardrail:
            execution.metadata["guardrail"] = "output"
    ...
    def wrap_model_call(self, request: ModelRequest, call_next: Any) -> Any:
        try:
            ...
            for model in self.models:
                try:
                    target = request.model_adapter(model) if request.model_adapter else model
                    response = target.invoke(request.messages, request.config)
                    request.execution.metadata["model_fallback"] = type(model).__name__
                    return response
    ...
    async def awrap_model_call(self, request: ModelRequest, call_next: Any) -> Any:
        try:
            ...
            for model in self.models:
                try:
                    target = request.model_adapter(model) if request.model_adapter else model
                    response = await target.ainvoke(request.messages, request.config)
                    request.execution.metadata["model_fallback"] = type(model).__name__
                    return response
                except Exception as exc:
                    last = exc
